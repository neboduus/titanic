...
# So we add a ceiling
fare_ceiling = 40 #soglia tariffa
# then modify the data in the Fare column to = 39, if it is greater or equal to the ceiling
data[ data[0::,9].astype(np.float) >= fare_ceiling, 9 ] = fare_ceiling - 1.0

fare_bracket_size = 10	#grandezza categoria tariffa 
number_of_price_brackets = fare_ceiling / fare_bracket_size # 4 categorie di tariffa (40/10=4)
...
# I can now find the stats(statistiche) of all the women and men on board
for i in xrange(number_of_classes):
    for j in xrange(number_of_price_brackets):
		#i+1 perché range(3) = 0,1,2 e le classi sono 1,2,3
		...
		#print "man_only_stats: ", man_only_stats
                                 #if i == 0 and j == 3:
...
# Also open a new file so I can write to it. 
predictions_file = open("SecondGenderClassModel_Predictions.csv", "wb")
